...
def test_write_creates_file(sample_template):
...
def test_sheet1_data_filled(sample_template):
    txns = [_txn("2026.03.27", "13:00", "바나프레소", 10300)]
    cls = [Classification(usage="팀 커피", expense_amount=10300, account="복리후생비[회식비]", companion="양경희,김보민", rule_number=1)]
    with tempfile.TemporaryDirectory() as d:
        out = os.path.join(d, "test.xlsx")
...
def test_sheet2_classification_filled(sample_template):
    txns = [_txn("2026.03.27", "13:00", "바나프레소", 10300)]
    cls = [Classification(usage="팀 커피", expense_amount=10300, account="복리후생비[회식비]", companion="양경희,김보민", rule_number=1)]
    with tempfile.TemporaryDirectory() as d:
        out = os.path.join(d, "test.xlsx")
...
def test_manual_items_leave_cells_empty(sample_template):
